...
def reset_candidate():
    ...
    # Do not pop widget-backed keys such as candidate_name, position,
    # interview_results or history_selector. Streamlit widgets own those
    # keys during a rerun. Clear the underlying values instead.
    for key in [
        "candidate_id",
        "job_text",
        "resume_text",
...
if supabase is not None:
    ...
    try:
        ...
        # Use a direct button action + rerun. This is more reliable here because
        # the sidebar also contains widget-backed candidate fields.
        if st.sidebar.button(
            "➕ Start New Candidate",
            use_container_width=True,
